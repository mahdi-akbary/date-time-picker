...
import {extendObject} from './helper.service';
...
export class NativeDateAdapter extends DateAdapter<Date> {
  ...
  getMonthNames(style: 'long' | 'short' | 'narrow', force?): string[] {
    if (this.lang == 'fa' && !force) {
      const months = [];
      this.JDateAdapter.getMonthNames(style).forEach(month => {
        switch (month) {
          case 'فروردین' :
            months.push('حمل');
            break;
  ...
  getDayOfWeekNames(style: 'long' | 'short' | 'narrow', force?): string[] {
    if (this.lang == 'fa' && !force) {
      const tempNameArray = this.JDateAdapter.getDayOfWeekNames(style);
      let index = tempNameArray.indexOf('آ');
      if (index !== -1) {
        tempNameArray[index] = 'ج';
      } else {
        index = tempNameArray.indexOf('آدینه');
        if (index !== -1) {
          tempNameArray[index] = 'جمعه';
        }
      }
