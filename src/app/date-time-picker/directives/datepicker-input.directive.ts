...
@Directive({
  selector: 'input[npaMatDatepicker]',
...
export class DatepickerInputDirective<D> implements AfterContentInit, ControlValueAccessor, OnDestroy,
  ...
  @Input()
  set mdDatepicker(value: DateTimePickerComponent<D>) {
    if (value) {
  ...
  _datepicker: DateTimePickerComponent<D>;

  @Input()
  set matDatepicker(value: DateTimePickerComponent<D>) {
  ...
  ngAfterContentInit() {
    if (this._datepicker) {
      this._datepickerSubscription =
          this._datepicker._selectedChanged.subscribe((selected: D) => {
            this.value = selected;
